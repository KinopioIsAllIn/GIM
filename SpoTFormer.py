...
class MSSTGT(nn.Module):
    def __init__(self, *, seq_len, num_classes, dim, depth, heads, mlp_dim, channels=3, dim_head=64, dropout=0., emb_dropout=0.):
        ...
        self.spatial_pos_embedding = nn.Parameter(torch.randn(3, dim))
    ...
    def forward(self, x):
        B, L, T, V, C = x.shape
        N = B * L
        x = x.reshape(-1, T, V, C).permute(0, 2, 1, 3).contiguous().reshape(-1, V*T, C)  # N, V*T, C
        # N, C, T, V = x.size()
        ...
        x = self.transformer(x)[:, 0, 0]  # 去掉一维 b, 1, 1, c -> b, 1, c
