...
#     temp = []
#     for i in range(len(tList)):  # 对于每一个动作类别，都用相同的list, 因为都是用attention score筛选的。
#         c_temp = []
#         temp_list = np.array(tList[i])[0]
#         if temp_list.any():
#             grouped_temp_list = grouping(temp_list)  # 相邻的进行分组
#             for j in range(len(grouped_temp_list)):  # 第几组
#                 inner_score = np.mean(wtcam[grouped_temp_list[j], i, 0])  # 计算组内分数，组内的动作分类分数
#
#                 len_proposal = len(grouped_temp_list[j])
#                 outer_s = max(
...
#                         range(int(grouped_temp_list[j][-1] + 1), outer_e + 1))  # 扩张后的
#
#                 if len(outer_temp_list) == 0:
#                     outer_score = 0
#                 else:
#                     outer_score = np.mean(wtcam[outer_temp_list, i, 0])  # 计算组外分数
#
#                 if loss_type == "oic":
#                     c_score = inner_score - outer_score  # + gamma * final_score[c_pred[i]] ##########################
#                 else:
...
#
#                 c_temp.append([c_pred[i], c_score, t_start, t_end])
#                 # if t_start > 10 and t_end < num_segments - 10:
#                 #     print(np.squeeze(cas_temp_atn[t_start - 10: t_end + 10]))
#                 #     print([vid_name, c_pred[i], c_score, t_start, t_end])
#             temp.append(c_temp)
#     return temp
...
def get_proposal_oic_2(tList,
    ...
    temp = []
    for i in range(len(tList)):  # 对于每一个动作类别，都用相同的list, 因为都是用attention score筛选的。
        # 我要改变的是分数信息。分数信息应该基于attention score计算
        c_temp = []
        temp_list = np.array(tList[i])[0]
        if temp_list.any():
            grouped_temp_list = grouping(temp_list)  # 相邻的进行分组
            for j in range(len(grouped_temp_list)):  # 第几组
                inner_score = np.mean(cas_temp_atn[grouped_temp_list[j], 0, 0])  # 计算组内分数，组内的动作分类分数

                apex_index = np.argmax(cas_temp_atn[grouped_temp_list[j], 0, 0]) + grouped_temp_list[j][0]
                action_score = wtcam[apex_index, i, 0]
                ...
                if len(outer_temp_list) == 0:
                    ...
                    outer_score = np.mean(cas_temp_atn[outer_temp_list, 0, 0])  # 计算组外分数

                if loss_type == "oic":
                    c_score = inner_score - outer_score + gamma * action_score # gamma * final_score[c_pred[i]] ##########################
                else:
                    c_score = inner_score
                t_start = grouped_temp_list[j][0]
                t_end = (grouped_temp_list[j][-1] + 1)
