    ...
    for i in range(np.shape(segment_predict)[0]):
        ...
        for a in ambilist:
            if a[0] == vn:
                ...
                # )
                gt = range(
                    int(round(float(a[2]))), int(round(float(a[3])))
                )
...
class ANETdetection(object):
    ...
    def _import_ground_truth(self, annotation_path):
        ...
        if self.args.dataset_name == 'CASME3':
            # subset_ind = [v[:-2] == self.args.validation_subject[7:] for v in videoname]
            subset_ind = [v[:-2] == self.args.validation_subject for v in videoname]

        else:
            subset_ind = [v[:8] == self.args.validation_subject for v in videoname]
        ...
        video_lst, t_start_lst, t_end_lst, label_lst, t_apex_lst = [], [], [], [], []
        ...
        for i in range(len(gtsegments)):
            for j in range(len(gtsegments[i])):
                ...
                t_start_lst.append(round(gtsegments[i][j][0]))
                t_end_lst.append(round(gtsegments[i][j][1]))
                t_apex_lst.append(round(gtsegments[i][j][2]))
    ...
    def wrapper_compute_average_precision(self):
        ...
        """
        tp = np.zeros((len(self.tiou_thresholds), 2))
        fp = np.zeros((len(self.tiou_thresholds), 2))
        gt = np.zeros((len(self.tiou_thresholds), 2))  # len(activity_index)
        apex_diff = np.zeros((len(self.tiou_thresholds), 2))
        ...
        for i, cidx in enumerate(self.activity_index.values()):
            ...
            fp[:, cidx] = results[i][1]
            gt[:, cidx] = results[i][2]
            apex_diff[:, cidx] = results[i][3]
        print(tp, fp, gt, apex_diff)
        return tp, fp, gt, apex_diff
    ...
    def evaluate(self):
        ...
        self.tp, self.fp, self.gt, self.apex_diff = self.wrapper_compute_average_precision()
        # print(self.ap)
        ...
        # if self.verbose:
            ...
            # print("Average-mAP: {}\n".format(self.mAP))
        return self.tp, self.fp, self.gt, self.apex_diff
...
def compute_average_precision_detection(
    ...
    for idx, this_pred in prediction.iterrows():
        try:
            ...
            ground_truth_videoid = ground_truth_gbvn.get_group(this_pred["video-id"][0])
        ...
        for tidx, tiou_thr in enumerate(tiou_thresholds):
            for jdx in tiou_sorted_idx:
                if tiou_arr[jdx] < tiou_thr: # if highest IOU < 0.5, pass
                    fp[tidx, idx] = 1
                    break
                if lock_gt[tidx, this_gt.loc[jdx]["index"]] >= 0: # if this gt already has a tp pred, move to next gt
                    continue
                # Assign as true positive after the filters above.
                tp[tidx, idx] = 1
                # print(this_pred[["t-start", "t-end", "t-apex"]].values, this_gt.loc[jdx]["t-start"], this_gt.loc[jdx]["t-end"], this_gt.loc[jdx]["t-apex"])
                # calculate apex diff
                # print(abs(this_gt.loc[jdx]["t-apex"] - this_pred["t-apex"]) / (this_gt.loc[jdx]["t-end"] - this_gt.loc[jdx]["t-start"] + 1))
                apex_diff += abs(this_gt.loc[jdx]["t-apex"] - this_pred["t-apex"]) / (this_gt.loc[jdx]["t-end"] - this_gt.loc[jdx]["t-start"] + 1)
    ...
    # f1_score = 2*recall_cumsum*precision_cumsum/(recall_cumsum+precision_cumsum)

    return tp_cumsum[0, -1], fp_cumsum[0, -1], npos, apex_diff
